...
interest are cosmological quantities such as the dark matter density
...
distribution.  In our case of analyzing the data from the  \textit{Dark Energy Survey} (DES), there are 10 distinct galaxy populations to
be characterized, each with $n(z)$ tabulated at $\approx100$ values, leading to $N=O(1000)$ parameters $n_k$ to be considered.
...
\citet{y6model} and the \texttt{Cosmosis} software\footnote{\url{https://cosmosis.readthedocs.io/en/latest/}} \citet{cosmosis}.

\begin{figure}
  \center
  \includegraphics[width=0.6\textwidth]{bin4_violins.pdf}
\caption{Violin plots for the redshift probability distribution $n(z)$
  of galaxies in lens bin 4 for the DES Y6 analysis.  The blue regions
  ...
  into 3 modes with coefficients $\vecu,$ then (2) drawing values of
  $\widetilde u_i$ from unit normal distributions and transforming each
  component $u_i$ to match the 1d distribution of the input samples' $u_i.$.  The dashed
...
\caption{At left: The size of the $\chi^2$ of modeling error attributable to compressing the
  ...
  matrix $\matD$ in \eqq{eq:DE} are plotted vs redshift.  Each mode is
  ...
  observable consequences of decreasing statistical significance.}
...
$\chi^2<0.025$ is attained with $M=3$ modes for this bin.  The modes
$U_i(z),$ \ie\ the rows of the decompression matrix $\matD,$ are
plotted on the right of Figure~\ref{fig:chiresid}.  Recall that each mode will be multiplied by
...
3000 input samples.  Some are substantially non-Gaussian,
...
\section{Summary}
  ...
  marginalization over the distribution of $\vecn$ even in for
