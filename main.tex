...
the samples of \vecn, which we write as $\{\vecn_\alpha\}$ for $\alpha\in
  1\ldots N_{\rm samp},$ are \ed{assigned to equally-spaced grid points} within some $M$-dimensional 
...
any  \vecu, \ed{\ie\ nearest-neighbor interpolation.}
\ed{In the example
...
$\hatc(\vecq,\vecn)$ and a fixed covariance matrix $\covm_c,$ \ed{known \textit{a
...
\ed{Inferences use the likelihood of the observed $\vecc$ under the
...
\vecn\ \ed{under a Gaussian likelihood \citep{TTH}.}  In many cases this matrix will be
...
this method in the Introduction.
  \ed{But the compression method is also more robust in other ways: it defines
...
%
\ed{Even in the case where all three assumptions of the covariance-inflation
method hold true, 
...
distribution of \vecu\ to see how the data have \ed{altered the prior
...
The observable quantities \ed{are the autocorrelation $w(\theta)$ of the 
...
\ed{The model $\hatc(\vecq,\vecn)$ for the observables consists of integrations
...
\caption{Violin plots for the redshift probability distribution $n(z)$
  ...
  [Although the $n(z)$ functions are calculated \ed{for $0<z<4,$} we
...
of the \ed{first} three modes appears to effect some combination of a $z$ shift
...
\ed{The means of the two distributions (centers of the violins)
...
DES model for this test).\footnote{\ed{Please note that this $\chi^2$ quantity
...
\caption{The histograms show the deviations of the predicted observable $w(\theta)$ quantities in
  ...
  on average, than the input distribution has. \ed{[Note the logarithmic horizontal
    axis.]} }
\label{fig:chihist}
\end{figure}

Figure~\ref{fig:chihist} shows the results: the \ed{distribution of the $\chi^2$
...
\ed{Note that this agreement indicates that the approximation of linearity of the
...
\ed{distinct} bins' $n(z)$ \ed{values,} and the compression process described
...
\section{Summary}
  ...
  \ed{The compression method is also robust to any nonlinearities
...
LCF/BQ/PI23/11970028.  \ed{We also thank the anonymous referee for pointing out
